...
```sh
git clone https://https://gitlab.teracloud.ninja/teracloud/pod-services/baas-spike/core
cd core
```
</details>
...
This section will guide you to setup a fully-functional local copy of `core`
...
from [`core's` Makefile][makefile-file] (most of which are
OS-independent)!

### Installing dependencies

To install all dependencies associated with `core`, run the
command
...
The pre-commit hooks used by `core` are located within the
...
Tests in `core` are classified as *fast* and *slow* - depending
...
|      `run`     	| Run *core*                           	|       NA      	|
|  `docker-gen`  	| Create production docker image for *core* 	|     docker    	|
|  `docker-debug`  	| Create debug-friendly docker image for *core* 	|     docker    	|
...
```text
core_<version>_<target-os>_<architecture>.<extension>
```
...
```sh
core_v1.2.1_windows_x86_64.exe
```
...
To run `core` in a docker container, read the instructions in
...
`core` is powered by a template generated using [`go-template`][go-template-link]
