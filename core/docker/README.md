...
```bash
make docker-gen IMAGE=core VERSION=latest
```
...
```bash
make clean-docker IMAGE=core VERSION=latest
```


## Debug Images

The [dockerfile](./Dockerfile) for *core* uses
...
```bash
make docker-debug IMAGE=core-debug VERSION=latest
```
...
```bash
make clean-docker IMAGE=core-debug VERSION=latest
```
